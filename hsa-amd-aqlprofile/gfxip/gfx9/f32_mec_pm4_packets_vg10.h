...
typedef struct PM4_MEC_COND_EXEC {
  ...
  union {
    struct {
      ...
      MEC_COND_EXEC_cache_policy_enum cache_policy : 1;
...
enum MEC_DMA_DATA_sas_enum { sas__mec_dma_data__memory = 0 };

enum MEC_DMA_DATA_das_enum { das__mec_dma_data__memory = 0 };
...
enum MEC_DMA_DATA_FILL_MULTI_src_sel_enum { src_sel__mec_dma_data_fill_multi__data = 2 };
...
// for the following events only: Sample_PipelineStats
//
...
typedef struct PM4_MEC_EVENT_WRITE_QUERY {
  union {
    PM4_MEC_TYPE_3_HEADER header;  /// header
    uint32_t ordinal1;
  };
...
} PM4MEC_EVENT_WRITE_QUERY, *PPM4MEC_EVENT_WRITE_QUERY;
#endif
...
enum MEC_INDIRECT_BUFFER_PASID_cache_policy_enum {
  ...
  cache_policy__mec_indirect_buffer_pasid__bypass = 2
};
...
#ifndef PM4_MEC_MEM_SEMAPHORE_DEFINED
#define PM4_MEC_MEM_SEMAPHORE_DEFINED
...
enum MEC_SECURE_CONTROL_command_enum { command__mec_secure_control__clear_tmz_memory = 0 };
