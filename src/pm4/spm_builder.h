...
class GpuSpmBuilder : public SpmBuilder, protected Builder, protected Primitives {
 public:
  void Begin(CmdBuffer* cmd_buffer, const SpmConfig* config, const counters_vector& counters_vec) {
    ...
    // SE: SX, TA, TD, TCP, SPI
    // RLC_SPM_<BLK>_PERFMON_SAMPLE_DELAY
    ...
    for (const auto& counter_des : counters_vec) {
      ...
      if (block_info->attr & CounterBlockSeAttr) {
        se_count += 1;
        Builder::BuildWriteUConfigRegPacket(cmd_buffer, Primitives::RLC_SPM_SE_MUXSEL_DATA__ADDR,
                                            Primitives::rlc_spm_muxsel_data(counter_des, counter_des));
      } else {
        global_count += 1;
        Builder::BuildWriteUConfigRegPacket(cmd_buffer, Primitives::RLC_SPM_GLOBAL_MUXSEL_DATA__ADDR,
                                            Primitives::rlc_spm_muxsel_data(counter_des, counter_des));
      }
    ...
    Builder::BuildWriteUConfigRegPacket(cmd_buffer, Primitives::RLC_SPM_GLOBAL_MUXSEL_DATA__ADDR, 0xffff);
    Builder::BuildWriteUConfigRegPacket(cmd_buffer, Primitives::RLC_SPM_GLOBAL_MUXSEL_ADDR__ADDR, 0);
    Builder::BuildWriteUConfigRegPacket(cmd_buffer, Primitives::RLC_SPM_SE_MUXSEL_ADDR__ADDR, 0);
    // Set segment size
