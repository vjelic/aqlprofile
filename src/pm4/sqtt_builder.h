...
class GpuSqttBuilder : public SqttBuilder, protected Builder, protected Prim {
 public:
  void begin(CmdBuffer* cmdBuff, const ThreadTraceConfig* config) {
    ...
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::GRBM_GFX_INDEX_ADDR, Prim::grbm_broadcast_value());
    ...
    const uint32_t mask_value = (config->mask) ? config->mask : Prim::sqtt_mask_value(config->targetCu, config->vmIdMask);
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_THREAD_TRACE_MASK_ADDR, mask_value);
    // Program the thread trace Perf mask
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_THREAD_TRACE_PERF_MASK_ADDR, Prim::sqtt_perf_mask_value());
    // Program the thread trace token mask
    const uint32_t token_mask_value = (config->tokenMask) ? config->tokenMask : Prim::sqtt_token_mask_value();
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_THREAD_TRACE_TOKEN_MASK_ADDR, token_mask_value);
    ...
    const uint32_t token_mask2_value = (config->tokenMask2) ? config->tokenMask2 : Prim::sqtt_token_mask2_value();
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_THREAD_TRACE_TOKEN_MASK2_ADDR, token_mask2_value);
    // Program the thread trace mode register, mode OFF
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_THREAD_TRACE_MODE_ADDR, Prim::sqtt_mode_off_value());
    // Program the HiWaterMark register to support stalling
    if (Prim::sqtt_stalling_enabled(mask_value, token_mask_value)) {
      Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_THREAD_TRACE_HIWATER_ADDR, Prim::SQ_THREAD_TRACE_HIWATER_VAL);
    }
    ...
    for (int se_index = 0; se_index < config->se_number; ++se_index, base_addr += base_step) {
      ...
      Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::GRBM_GFX_INDEX_ADDR, Prim::grbm_se_sh_index_value(se_index, 0));
      // Program base address of buffer to use for thread trace
      Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_THREAD_TRACE_BASE_ADDR, Prim::sqtt_base_value(base_addr));
      // Program the size of thread trace buffer
      Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_THREAD_TRACE_SIZE_ADDR, Prim::sqtt_size_value(config->data_buffer_size));
      // Program the thread trace ctrl register
      Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_THREAD_TRACE_CTRL_ADDR, Prim::sqtt_ctrl_value());
    }
    ...
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_THREAD_TRACE_MODE_ADDR, Prim::sqtt_mode_on_value());
    // Issue a CSPartialFlush cmd including cache flush
    Builder::BuildWriteWaitIdlePacket(cmdBuff);
  }
  ...
  void end(CmdBuffer* cmdBuff, const ThreadTraceConfig* config) {
    ...
    for (int se_index = 0; se_index < config->se_number; se_index++) {
      ...
      Builder::BuildCopyDataPacket(cmdBuff, Prim::COPY_DATA_SEL_SRC_SYS_PERF_COUNTER_PRM, Prim::SQ_THREAD_TRACE_STATUS_ADDR,
                          0, control_buffer + statusIdx, Prim::COPY_DATA_SEL_COUNT_1DW_PRM, true);
      const uint32_t cntrIdx = ((TT_STATUS_IDX_MAX * se_index) + TT_STATUS_IDX_CNTR);
      Builder::BuildCopyDataPacket(cmdBuff, Prim::COPY_DATA_SEL_SRC_SYS_PERF_COUNTER_PRM, Prim::SQ_THREAD_TRACE_CNTR_ADDR,
                          0, control_buffer + cntrIdx, Prim::COPY_DATA_SEL_COUNT_1DW_PRM, true);
      const uint32_t wptrIdx = ((TT_STATUS_IDX_MAX * se_index) + TT_STATUS_IDX_WPTR);
      Builder::BuildCopyDataPacket(cmdBuff, Prim::COPY_DATA_SEL_SRC_SYS_PERF_COUNTER_PRM, Prim::SQ_THREAD_TRACE_WPTR_ADDR,
                          0, control_buffer + wptrIdx, Prim::COPY_DATA_SEL_COUNT_1DW_PRM, true);
    }
    // Reset the GRBM to broadcast mode
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::GRBM_GFX_INDEX_ADDR, Prim::grbm_broadcast_value());
    ...
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_THREAD_TRACE_SIZE_ADDR, Prim::sqtt_zero_size_value());
    // Program the thread trace ctrl register
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_THREAD_TRACE_CTRL_ADDR, Prim::sqtt_ctrl_value());
    // Issue a CSPartialFlush cmd including cache flush
    Builder::BuildWriteWaitIdlePacket(cmdBuff);
