...
class GpuPmcBuilder : public PmcBuilder, protected Builder, protected Prim {
 public:
  ...
  void begin(CmdBuffer* cmdBuff, const counters_vector& countersVec) {
    ...
    if (Prim::GFXIP_LEVEL == 9) Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::RLC_PERFMON_CLK_CNTL_ADDR, 1);
    // Reset the counter list
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::CP_PERFMON_CNTL_ADDR, Prim::cp_perfmon_cntl_reset_value());
    ...
    for (const auto & counter_des: countersVec) {
      ...
      if (block_info->to_clean_regs) {
        for (uint32_t i = 0; i < block_info->counter_count; ++i) {
          Builder::BuildWriteUConfigRegPacket(cmdBuff, block_info->counter_reg_info[i].register_addr_lo, 0);
          Builder::BuildWriteUConfigRegPacket(cmdBuff, block_info->counter_reg_info[i].register_addr_hi, 0);
        }
      }
      if (block_info->select_value != NULL) {
        Builder::BuildWriteUConfigRegPacket(cmdBuff, reg_info.select_addr, block_info->select_value(counter_des));
      }
      if (block_info->sq_block) {
        Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::SQ_PERFCOUNTER_MASK_ADDR, Prim::sq_mask_value(counter_des));
        Builder::BuildWriteUConfigRegPacket(cmdBuff, reg_info.control_addr, Prim::sq_control_value(counter_des));
      }
    ...
    Builder::BuildWriteShRegPacket(cmdBuff, Prim::COMPUTE_PERFCOUNT_ENABLE_ADDR, Prim::cp_perfcount_enable_value());
    // Reset the counter list
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::CP_PERFMON_CNTL_ADDR, Prim::cp_perfmon_cntl_reset_value());
    // Start the counter list
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::CP_PERFMON_CNTL_ADDR, Prim::cp_perfmon_cntl_start_value());
  ...
  uint32_t end(CmdBuffer* cmdBuff, const counters_vector& countersVec, void* dataBuff) {
    ...
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::CP_PERFMON_CNTL_ADDR, Prim::cp_perfmon_cntl_stop_value());
    // Reset Grbm to its default state - broadcast
    Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::GRBM_GFX_INDEX_ADDR, Prim::grbm_broadcast_value());
    ...
    for (const auto & counter_des: countersVec) {
      ...
      for (uint32_t se_index = 0; se_index < se_index_end; ++se_index) {
        ...
        Builder::BuildCopyDataPacket(cmdBuff, Prim::COPY_DATA_SEL_REG_PRM, reg_info.register_addr_lo, 0,
                            (uint32_t*)dataBuff + read_counter, Prim::COPY_DATA_SEL_COUNT_1DW_PRM, false);
        Builder::BuildCopyDataPacket(cmdBuff, Prim::COPY_DATA_SEL_REG_PRM, reg_info.register_addr_hi, 0,
    ...
    if (Prim::GFXIP_LEVEL == 9) Builder::BuildWriteUConfigRegPacket(cmdBuff, Prim::RLC_PERFMON_CLK_CNTL_ADDR, 0);
