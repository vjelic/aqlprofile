...
#ifndef _GFX11_PM4DEFS_H_
#define _GFX11_PM4DEFS_H_
...
#define GFX11_PM4DEFS_VERSION_MAJOR  1
#define GFX11_PM4DEFS_VERSION_MINOR  1
...
// PM4 type 2 NOP
#define PM4_TYPE_2_NOP        ((unsigned int)(PM4_TYPE_2 << PM4_TYPE_SHIFT))
...
typedef union _CONTEXT_CONTROL_ENABLE
{
    struct
    {
        ...
        unsigned int enableUserConfigReg__CI__VI    :  1;   ///< User Config Reg on CI/VI(SI-Rsvd)
...
#define PREAMBLE_CNTL_BEGIN_PREAMBLE        0
#define PREAMBLE_CNTL_END_PREAMBLE          1
...
typedef struct _PM4CMDDRAWINDEXTYPE
{
    ...
    union
    {
        unsigned int        vgtDmaIndexType;   ///< See VGT_DMA_INDEX_TYPE register for details
        unsigned int        ordinal2;
    };
...
typedef struct _PM4CMDWAITREGMEM
{
    ...
    union
    {
        struct
        {
            ...
            unsigned int    reserved2       : 15; ///< reserved
...
typedef struct _PM4CMDEVENTWRITE
{
    ...
    union
    {
        struct
        {
            ...
            unsigned int    reserved4        : 7;   ///< reserved
        };
        unsigned int        ordinal2;
    };
...
typedef struct _PM4CMDEVENTWRITEQUERY
{
    ...
    // Sample_PipelineStats, Sample_StreamoutStats, and Zpass (Occlusion on SI).

    // PIXEL_PIPE_STAT_CONTROL event (CI+) only require ordinal 3.
    // PIXEL_PIPE_STAT_DUMP event (CI+) requires both ordinal 3 and 4
    ...
    union
    {
        ...
        {
            unsigned int reserved3      : 3;
            unsigned int counterId      : 6;
            unsigned int stride         : 2;
            unsigned int instanceEnable : 16;
    ...
    union
    {
        struct
        {
            ...
            unsigned int    reserved5   : 16;   ///< reserved
        };
        unsigned int        ordinal4;
    };
...
#define EVENT_WRITE_INDEX_OCCLUSION_EVENT           1 // Event index for ZPASS_DONE (SI only)
                                                      // PIXEL_PIPE_STAT_CONTROL (CI+)
                                                      // PIXEL_PIPE_STAT_DUMP (CI+)
...
typedef struct _PM4CMDCONDWRITE
{
    ...
    union
    {
        struct
        {
            ...
            unsigned int    reserved2   :   3;
            unsigned int    writeSpace  :   1;  ///< write space
            unsigned int    reserved3   :  23;
        };
        unsigned int    ordinal2;
    };
    union
...
typedef struct _PM4CMDRELEASEMEM
{
    ...
    union
    {
        struct
        {
            ...
            unsigned int    eventIndex   : 4;    ///< event index
            unsigned int    reserved2    : 3;    ///< reserved
            unsigned int    writeBackL2  : 1;    ///< write back L2 cache
            unsigned int    invalidateL1 : 1;    ///< invalidate L1 cache
            unsigned int    invalidateL2 : 1;    ///< invalidate L2 cache
            unsigned int    reserved3    : 14;   ///< reserved
        };
        unsigned int        ordinal2;
    };
    union
    {
        ...
        {
            unsigned int    reserved4   : 16;   ///< reserved
            unsigned int    dstSel      : 2;    ///< destination of data
            unsigned int    reserved5   : 6;    ///< reserved
            unsigned int    intSel      : 3;    ///< select interrupt action for event types
            unsigned int    reserved6   : 2;    ///< reserved
            unsigned int    dataSel     : 3;    ///< select source of data
        };
        unsigned int        ordinal3;
    };
    union
    ...
    union
    {
        unsigned int        dataLo;             ///< value that will be written to memory
        unsigned int        ordinal6;
    };
    union
    {
        unsigned int        dataHi;             ///< value that will be written to memory
        unsigned int        ordinal7;
    };
    union
    {
        unsigned int        ctxId;              ///< context Id
        unsigned int        ordinal8;
    };
...
#define PM4_CMD_INDIRECT_BUFFER_PRIV_DWORDS                   \
    (sizeof(PM4CMDINDIRECTBUFFER) / sizeof(unsigned int))
...
#define PM4_CMD_ATOMIC_DWORDS                                  \
    (sizeof(PPM4CMDATOMIC) / sizeof(unsigned int))
...
#define PM4_CMD_ACQUIRE_MEM_DWORDS                               \
    (sizeof(PM4CMDACQUIREMEM) / sizeof (unsigned int))
