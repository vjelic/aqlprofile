...
class gfx8_cntx_prim {
  ...
  const static uint32_t SQ_THREAD_TRACE_STATUS_OFFSET = mmSQ_THREAD_TRACE_STATUS__VI - UCONFIG_SPACE_START__CI__VI;
  ...
  static bool sqtt_stalling_enabled(const uint32_t & mask_val, const uint32_t & token_mask_val) {
    ...
    return ((mask.bits.SQ_STALL_EN__CI__VI) ||
            (mask.bits.SPI_STALL_EN__CI__VI) ||
            (mask.bits.REG_STALL_EN__CI__VI) ||
            (token_mask.bits.REG_DROP_ON_STALL__CI__VI));
  }
