...
hsa_status_t AgentInfo::get_agent_handle_cb(hsa_agent_t agent, void* userdata)
{
    ...
    aqlprofile_pmc_event_t gl1c {.block_index = 0, .flags = flags, .block_name = HSA_VEN_AMD_AQLPROFILE_BLOCK_NAME_GL1C};
    ...
    {
        info->add_event(gl1c, "GL1C_REQ", 32, 14);
        info->add_event(gl1c, "GL1C_READ", 32, 21);
        info->add_event(gl1c, "GL1C_WRITE", 32, 28);
